...
class AuthController(private val authService: AuthService) {
    @PostMapping("/login")
    fun login(@RequestBody() userData: AuthUserDto): BasicSuccessfulResponse<UserTokensDto> = authService.login(userData)

    @PostMapping("/register")
    fun register(@RequestBody() userData: AuthUserDto): BasicSuccessfulResponse<UserTokensDto> = authService.register(userData)
}